...
export class ReaderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Performs the actual height measurement and adjustment
  private synchronizeColumnHeights(): void {
    if (this.isSyncingHeights || this.currentParallelMode !== 'side' || !this.readerContentRef?.nativeElement) {
      return; // Prevent recursion or running when not needed
    }

    this.isSyncingHeights = true;
    console.log("Synchronizing column heights...");
    const contentElement = this.readerContentRef.nativeElement;

    // Find all block containers generated by the pipe
    const blockContainers = contentElement.querySelectorAll<HTMLDivElement>('.sbs-block-container');

    if (!blockContainers || blockContainers.length === 0) {
      console.warn("Height Sync: No '.sbs-block-container' elements found.");
    ...
    blockContainers.forEach((container, index) => {
      const engCol = container.querySelector<HTMLDivElement>('.sbs-eng-column');
      const ukrCol = container.querySelector<HTMLDivElement>('.sbs-ukr-column');

      if (engCol && ukrCol) {
        // Reset heights first to get natural scrollHeight
        engCol.style.minHeight = 'auto';
        ukrCol.style.minHeight = 'auto';

        // Force browser reflow to recalculate heights accurately
        // Reading scrollHeight forces the reflow
        const engHeight = engCol.scrollHeight;
        const ukrHeight = ukrCol.scrollHeight;
        const maxHeight = Math.max(engHeight, ukrHeight);

        // console.log(`Block ${index + 1}: EngH=${engHeight}, UkrH=${ukrHeight}, MaxH=${maxHeight}`); // Detailed log

        // Apply the max height as min-height to both columns if needed
        // Add a small threshold (e.g., 1px) to avoid unnecessary changes for tiny differences
        if (Math.abs(engHeight - maxHeight) > 1) {
          engCol.style.minHeight = `${maxHeight}px`;
          changesMade = true;
        }
        if (Math.abs(ukrHeight - maxHeight) > 1) {
          ukrCol.style.minHeight = `${maxHeight}px`;
          changesMade = true;
        }

      } else {
        console.warn(`Height Sync: Missing columns in block container index ${index}.`);
      }
    });

    console.log(`Height synchronization complete. Changes applied: ${changesMade}`);
    this.isSyncingHeights = false;

    // If heights were changed, the overall scroll dimensions might change,
    // so update scroll state and potentially re-measure chapters if alignment affects them.
    if (changesMade) {
      this.ngZone.run(() => { // Run updates in Angular zone
        this.updateScrollState();
        // Optionally re-measure chapters if height changes could affect their offsets significantly
        // this.scheduleChapterOffsetMeasurement(); // Be cautious, might cause loops if not careful
