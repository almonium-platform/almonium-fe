/**
 * Writes src/assets/i18n/pseudo.json from the extracted source messages: every string accented, stretched and
 * bracketed, so one pass through the app in the pseudo-locale shows which layouts a longer language would
 * break. `npm run i18n` extracts first and then runs this.
 *
 *   [Šéţţîñĝš~~~~]     ← accents catch text that never went through $localize; the tail is the German-length
 *                        surplus; a missing bracket is a clipped end.
 *
 * Placeholders (`{$INTERPOLATION}`, `{$START_TAG_STRONG}`) and the keywords of ICU expressions are left alone;
 * only their case bodies are stretched.
 */
...
const SOURCE = join(root, 'src/locale/messages.json');
const TARGET = join(root, 'src/assets/i18n/pseudo.json');
...
function stretch(text) {
  ...
  while (i < text.length) {
    ...
    if (inner.startsWith('$') || secondComma < 0) {
      out += text.slice(i, close + 1); // a placeholder, or something we do not understand: verbatim
    } else {
      // An ICU expression: `{VAR, plural, =1 {one} other {many}}` keeps its head and keys, stretches its bodies.
      let body = '';
...
async function main() {
  ...
  const stretched = Object.fromEntries(Object.entries(translations).map(([id, message]) => [id, pseudo(message)]));
  await mkdir(dirname(TARGET), {recursive: true});
  await writeFile(TARGET, `${JSON.stringify({locale: 'pseudo', translations: stretched}, null, 2)}\n`);
